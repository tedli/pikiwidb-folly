...
[[gnu::weakref("__cxa_demangle")]] static char *
cxxabi_demangle(char const *, char *, size_t *, int *);
...
FOLLY_MAYBE_UNUSED FOLLY_ERASE int
rust_demangle_callback_fallback(const char *, int, demangle_callbackref,
                                void *) {
  return 0;
}

FOLLY_CREATE_QUAL_INVOKER(invoke_rust_demangle_primary,
                          ::rust_demangle_callback);
FOLLY_CREATE_QUAL_INVOKER(invoke_rust_demangle_fallback,
                          rust_demangle_callback_fallback);

using invoke_rust_demangle_fn =
    folly::invoke_first_match<invoke_rust_demangle_primary,
                              invoke_rust_demangle_fallback>;
constexpr invoke_rust_demangle_fn invoke_rust_demangle;

int call_rust_demangle_callback(const char *mangled, int options,
...
fbstring demangle(const char *name) {
  ...
  if (folly::demangle_build_has_liberty) {
    ...
    for (auto func : funcs) {
      ...
      // failure
      int success = func(name, liberty_demangle_options, demangleStringCallback,
                         &demangled);
  ...
  if (cxxabi_demangle) {
    ...
    // other junk)
    return fbstring(demangled, strlen(demangled), len,
