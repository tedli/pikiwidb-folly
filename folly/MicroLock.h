...
protected:
  ...
  FOLLY_DISABLE_SANITIZERS static uint8_t
  lockSlowPath(uint32_t oldWord, detail::Futex<> *wordPtr, unsigned baseShift,
               unsigned maxSpins, unsigned maxYields) noexcept;
  ...
  static constexpr uint8_t decodeDataFromWord(uint32_t word,
                                              unsigned baseShift) noexcept {
    return static_cast<uint8_t>(static_cast<uint8_t>(word >> baseShift) >>
                                kNumLockBits);
  }
  uint8_t decodeDataFromWord(uint32_t word) const noexcept {
    return decodeDataFromWord(word, baseShift());
  }
  static constexpr uint32_t encodeDataToWord(uint32_t word,
...
inline void MicroLockCore::store(uint8_t value,
  ...
  while (true) {
    ...
    if (wordPtr->compare_exchange_weak(oldWord, newWord, order,
...
bool MicroLockBase<MaxSpins, MaxYields>::try_lock() noexcept {
  ...
  } while (!wordPtr->compare_exchange_weak(oldWord, oldWord | heldBit(),
...
uint8_t MicroLockBase<MaxSpins, MaxYields>::lockAndLoad() noexcept {
  ...
  if ((oldWord & heldBit()) == 0 &&
      wordPtr->compare_exchange_weak(oldWord, oldWord | heldBit(),
                                     std::memory_order_acquire,
                                     std::memory_order_relaxed)) {
