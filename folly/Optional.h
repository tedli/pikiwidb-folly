...
template <class Value> struct OptionalPromise;
template <class Value> struct OptionalPromiseReturn;
} // namespace detail
...
// Conversions to ease migration to std::optional
  ...
  template <typename U, typename = std::enable_if_t<
                            std::is_same_v<U, std::optional<Value>>>>
  ...
  explicit operator std::optional<Value>() && noexcept(
    ...
    std::optional<Value> ret =
        storage_.hasValue ? std::optional<Value>(std::move(storage_.value))
                          : std::nullopt;
    reset();
    return ret;
  }
  ...
  template <class Arg> Optional &operator=(Arg &&arg) {
  ...
  /// @methodset Modifiers
  template <class... Args> Value &emplace(Args &&...args) {
    reset();
  ...
  /// @methodset Getters
  template <class U> constexpr Value value_or(U &&dflt) const & {
  ...
  template <class U> constexpr Value value_or(U &&dflt) && {
    if (storage_.hasValue) {
      return std::move(storage_.value);
    }
...
template <class T> const T *get_pointer(const Optional<T> &opt) {
  return opt.get_pointer();
}

template <class T> T *get_pointer(Optional<T> &opt) {
  return opt.get_pointer();
}
...
template <class T> struct hash<folly::Optional<T>> {
...
namespace folly {
namespace detail {
template <typename Value> struct OptionalPromise;

template <typename Value> struct OptionalPromiseReturn {
...
template <typename Value> struct OptionalAwaitable {
  Optional<Value> o_;
