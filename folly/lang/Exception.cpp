...
public:
  ...
  virtual bool __do_catch(std::type_info const *thrown_type,
                          void **thrown_object, unsigned outer) const = 0;
  virtual bool __do_upcast(std::type_info const *target,
                           void **thrown_object) const = 0;
};
...
#if !defined(__clang__)
template <class _E> void *__GetExceptionInfo(_E); // builtin
#endif
...
std::type_info const *
exception_ptr_exception_typeid(std::exception const &ex) noexcept {
...
static decltype(auto) cxxabi_with_cxa_exception(void *object, F f) {
  if (cxxabi_cxa_exception_sans_reserve()) {
    ...
    auto exception =
        object ? static_cast<cxa_exception *>(object) - 1 : nullptr;
    return f(exception);
  } else {
...
template <typename T> static T *win32_decode_pointer(T *ptr) {
  return static_cast<T *>(
      DecodePointer(const_cast<void *>(static_cast<void const *>(ptr))));
}

static EHExceptionRecord *
win32_get_record(std::exception_ptr const &ptr) noexcept {
...
exception_shared_string::~exception_shared_string() { state_->ruin(); }
