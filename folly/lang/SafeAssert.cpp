...
ssize_t write(int fh, void const *buf, size_t count) {
  ...
  if ((r > 0 && size_t(r) != count) || (r == -1 && errno == ENOSPC)) {
    ...
    if (GetFileType(h) == FILE_TYPE_PIPE) {
      DWORD state = 0;
      if (GetNamedPipeHandleState(h, &state, nullptr, nullptr, nullptr, nullptr,
                                  0)) {
...
void writeStderr(const char *s) { writeStderr(s, strlen(s)); }
void flushStderr() { fileutil_detail::wrapNoInt(fsync, stderr_fileno); }

[[noreturn]] FOLLY_COLD void
safe_assert_terminate_v(safe_assert_arg const *arg_, int const error,
