...
using throw_exception_arg_ = //
    conditional_t<
        ...
        conditional_t<is_trivially_copyable_v<remove_cvref_t<R>>,
                      throw_exception_arg_trivial_, throw_exception_arg_base_>>;
template <typename R>
...
template <typename E, typename Try, typename Catch, typename... CatchA,
          typename R = std::common_type_t<
              ...
              decltype(FOLLY_DECLVAL(Catch &&)(FOLLY_DECLVAL(E &),
                                               FOLLY_DECLVAL(CatchA &&)...))>>
FOLLY_ERASE_TRYCATCH R catch_exception(Try &&t, Catch &&c, CatchA &&...a) {
#if FOLLY_HAS_EXCEPTIONS
  try {
    return static_cast<Try &&>(t)();
  } catch (E e) {
    return invoke_cold(static_cast<Catch &&>(c), e,
...
inline std::nullptr_t exception_ptr_nullptr() { return nullptr; }
...
auto exception_ptr_catching(std::exception_ptr const &ptr, Catch catch_) {
  ...
  return catch_exception([&] { return catch_exception<T>(try_, catch_); },
                         exception_ptr_nullptr);
}

std::type_info const *
exception_ptr_exception_typeid(std::exception const &) noexcept;

std::type_info const *
exception_ptr_get_type_(std::exception_ptr const &ptr) noexcept;

void *exception_ptr_get_object_(std::exception_ptr const &,
                                std::type_info const *) noexcept;
...
exception_ptr_get_type(std::exception_ptr const &ptr) noexcept {
...
//  not as caught.
inline void *
exception_ptr_get_object(std::exception_ptr const &ptr,
...
public:
  ...
  template <typename String, typename = decltype(test_params_(
 ...
 */
template <class T> class static_what_exception : public T {
protected:
