    ...
    if ((!d || ::folly::kIsDebug || ::folly::kIsSanitize) &&                   \
      ...
      static constexpr ::folly::detail::safe_assert_arg                        \
          __folly_detail_safe_assert_arg{                                      \
              u ? nullptr : #expr, __FILE__, FOLLY_PP_CONSTINIT_LINE_UNSIGNED, \
...
template <safe_assert_msg_type... A> struct safe_assert_msg_type_s {};
...
template <typename> struct safe_assert_msg_types;
template <safe_assert_msg_type... A>
...
template <bool P>
[[noreturn]] FOLLY_COLD FOLLY_NOINLINE void
safe_assert_terminate(safe_assert_arg const *arg,
                      ...) noexcept; // the true backing function

template <bool P> struct safe_assert_terminate_w {
