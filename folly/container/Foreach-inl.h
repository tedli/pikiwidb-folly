...
template <typename, typename T> struct IsMemberGetFound : std::false_type {};
template <typename T>
...
template <typename, typename T> struct IsTuple : std::false_type {};
template <typename T> struct IsTuple<EnableIfTuple<T>, T> : std::true_type {};
...
template <typename, typename T> struct IsRange : std::false_type {};
template <typename T> struct IsRange<EnableIfRange<T>, T> : std::true_type {};
...
void for_each_range_impl(index_constant<2>, Sequence &&range, Func &func) {
  ...
  auto three_arg_adaptor = [&func](auto &&ele, auto index,
                                   auto) -> decltype(auto) {
    return func(std::forward<decltype(ele)>(ele), index);
  };
  for_each_range_impl(index_constant<3>{}, std::forward<Sequence>(range),
                      three_arg_adaptor);
}
...
void for_each_tuple_impl(std::index_sequence<Indices...>, Sequence &&seq,
                         Func &func) {
  using _ = int[];
  ...
  void(_{(((control == loop_continue)
               ? (control = invoke_returning_loop_control(
                      func, get_impl<Indices>(std::forward<Sequence>(seq)),
...
void for_each_tuple_impl(index_constant<1>, Sequence &&seq, Func &func) {
  ...
  for_each_tuple_impl(index_constant<2>{}, std::forward<Sequence>(seq),
                      two_arg_adaptor);
}
...
decltype(auto) fetch_impl(RangeTag, Sequence &&sequence, Index &&index) {
  ...
  using tag = std::conditional_t<
      ...
      BeginAddTag, IndexingTag>;
  return fetch_impl(tag{}, std::forward<Sequence>(sequence),
...
constexpr decltype(auto) fetch(Sequence &&sequence, Index &&index) {
  namespace fed = for_each_detail;
  using tag = fed::SequenceTag<Sequence>;
  return for_each_detail::fetch_impl(tag{}, std::forward<Sequence>(sequence),
                                     std::forward<Index>(index));
}
