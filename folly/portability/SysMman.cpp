...
void *mmapWinArgs(void *addr, size_t length, int prot, int flags, int fd,
                  DWORD offHigh, DWORD offLow) {
  ...
  if (!(flags & MAP_ANONYMOUS) || (flags & MAP_SHARED)) {
    ...
    HANDLE fmh = CreateFileMapping(h, nullptr, newProt,
...
void *mmap(void *addr, size_t length, int prot, int flags, int fd, off_t off) {
  ...
  return mmapWinArgs(addr, length, prot, flags, fd, (DWORD)(0),
                     (DWORD)(off & 0xFFFFFFFF));
}

void *mmap64(void *addr, size_t length, int prot, int flags, int fd,
             off64_t off) {
  return mmapWinArgs(addr, length, prot, flags, fd,
