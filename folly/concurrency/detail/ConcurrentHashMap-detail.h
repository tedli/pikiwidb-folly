...
template <typename KeyType, typename ValueType, typename Allocator,
          template <typename> class Atom, typename Enabled = void>
class ValueHolder {
public:
...
class ValueHolder<
    KeyType, ValueType, Allocator, Atom,
...
public:
  ...
  ValueHolder(std::piecewise_construct_t, Arg &&k, Args &&...args) {
    ...
    new (item_) CountedItem(std::piecewise_construct, std::forward<Arg>(k),
...
template <typename Allocator> class HazptrDeleter {
public:
  template <typename Node> void operator()(Node *node) {
    node->~Node();
...
public:
  ...
  template <typename Table> void operator()(Table *table) {
...
class NodeT : public hazptr_obj_base_linked<
                  NodeT<KeyType, ValueType, Allocator, Atom>, Atom,
...
public:
  ...
  NodeT(hazptr_obj_cohort<Atom> *cohort, Arg &&k, Args &&...args)
      : item_(std::piecewise_construct, std::forward<Arg>(k),
              std::forward<Args>(args)...) {
    init(cohort);
  }
  ...
  template <typename S> void push_links(bool m, S &s) {
    if (m) {
...
template <typename KeyType, typename ValueType, uint8_t ShardBits = 0,
...
public:
  ...
  BucketTable(size_t initial_buckets, float load_factor, size_t max_size,
  ...
  bool insert(Iterator &it, size_t h, const K &k, InsertType type,
              MatchFunc match, hazptr_obj_cohort<Atom> *cohort,
              Args &&...args) {
    return doInsert(it, h, k, type, match, nullptr, cohort,
                    std::forward<Args>(args)...);
  }

  template <typename MatchFunc, typename K, typename... Args>
  bool insert(Iterator &it, size_t h, const K &k, InsertType type,
              MatchFunc match, Node *cur, hazptr_obj_cohort<Atom> *cohort) {
  ...
  template <typename K> bool find(Iterator &res, size_t h, const K &k) {
      ...
      while (node) {
        if (KeyEqual()(key, node->getItem().first)) {
          ...
          if (iter) {
            ...
            iter->setNode(node->next_.load(std::memory_order_acquire), buckets,
                          bcount, idx);
            iter->next();
          }
          decSize();
...
private:
  ...
  class Buckets
      : public hazptr_obj_base<Buckets, Atom,
  ...
  public:
    ...
    void destroy(size_t count) {
      ...
      this->~Buckets();
      Allocator().deallocate((uint8_t *)this,
...
public:
  ...
  public:
    ...
    void setNode(Node *node, Buckets *buckets, size_t bucket_count,
                 uint64_t idx) {
      node_ = node;
      buckets_ = buckets;
      idx_ = idx;
...
private:
  ...
  void getBucketsAndCount(size_t &bcount, Buckets *&buckets,
  ...
  template <typename MatchFunc, typename K, typename... Args>
  bool doInsert(Iterator &it, size_t h, const K &k, InsertType type,
                MatchFunc match, Node *cur, hazptr_obj_cohort<Atom> *cohort,
                Args &&...args) {
...
private:
  ...
  public:
    ...
    void setNodeAndTag(std::size_t index, Node *node, std::size_t tag) {
      FOLLY_SAFE_DCHECK(index < kCapacity &&
                            (tag == 0x0 || (tag >= 0x80 && tag <= 0xff)),
                        "");
    ...
    static constexpr uint64_t kSaturatedOutboundOverflowCount =
        0xffULL << kOutboundOverflowIndex;
    static constexpr uint64_t kOutboundOverflowOperand =
        0x1ULL << kOutboundOverflowIndex;
    ...
    void incrOutboundOverflowCount() {
      ...
      if (count < kSaturatedOutboundOverflowCount) {
        tags_hi_.store(count + kOutboundOverflowOperand,
  ...
  public:
    ...
    void destroy(size_t count) {
      ...
      this->~Chunks();
      Allocator().deallocate((uint8_t *)this,
                             sizeof(Chunk) * count + sizeof(*this));
    }
...
public:
  ...
  public:
    ...
    void setNode(Node *node, Chunks *chunks, size_t chunk_count,
                 uint64_t chunk_idx, uint64_t tag_idx) {
  ...
  SIMDTable(size_t initial_size, float load_factor, size_t max_size,
            hazptr_obj_cohort<Atom> *cohort)
      : load_factor_(load_factor), max_size_(max_size), chunks_(nullptr),
        chunk_count_(0) {
    DCHECK(cohort);
    DCHECK(max_size_ == 0 ||
           (isPowTwo(max_size_) &&
  ...
  bool insert(Iterator &it, size_t h, const K &k, InsertType type,
    ...
    if (!prepare_insert(it, k, type, match, cohort, chunk_idx, tag_idx, node,
                        chunks, ccount, hp)) {
      return false;
    }
...
private:
  ...
  // Must hold lock.
  template <typename K>
  Node *find_internal(const K &k, const HashPair &hp, Chunks *chunks,
                      size_t ccount, size_t &chunk_idx, size_t &tag_idx) {
  ...
  template <typename MatchFunc, typename K, typename... Args>
  bool prepare_insert(Iterator &it, const K &k, InsertType type,
                      MatchFunc match, hazptr_obj_cohort<Atom> *cohort,
                      size_t &chunk_idx, size_t &tag_idx, Node *&node,
  ...
  void rehash_internal(size_t new_chunk_count,
                       hazptr_obj_cohort<Atom> *cohort) {
  ...
  void getChunksAndCount(size_t &ccount, Chunks *&chunks,
                         hazptr_holder<Atom> &hazptr) {
    while (true) {
      auto seqlock = seqlock_.load(std::memory_order_acquire);
...
template <typename KeyType, typename ValueType, uint8_t ShardBits = 0,
          ...
          template <typename> class Atom = std::atomic,
          class Mutex = std::mutex,
          template <typename, typename, uint8_t, typename, typename, typename,
                    template <typename> class, class>
          class Impl = concurrenthashmap::bucket::BucketTable>
class alignas(64) ConcurrentHashMapSegment {
  using ImplT = Impl<KeyType, ValueType, ShardBits, HashFn, KeyEqual, Allocator,
                     Atom, Mutex>;
...
public:
  ...
  ConcurrentHashMapSegment(size_t initial_buckets, float load_factor,
                           size_t max_size, hazptr_obj_cohort<Atom> *cohort)
  ...
  bool insert(Iterator &it, size_t h, Key &&k, Value &&v) {
    ...
    auto res = insert_internal(
        it, h, node->getItem().first, InsertType::DOES_NOT_EXIST,
        [](const ValueType &) { return false; }, node);
    if (!res) {
      node->~Node();
      Allocator().deallocate((uint8_t *)node, sizeof(Node));
    }
  ...
  bool try_emplace(Iterator &it, size_t h, Key &&k, Args &&...args) {
    ...
    return insert_internal(
        it, h, std::forward<Key>(k), InsertType::DOES_NOT_EXIST,
        [](const ValueType &) { return false; }, std::forward<Key>(k),
        std::forward<Args>(args)...);
  }
  ...
  bool emplace(Iterator &it, size_t h, const KeyType &k, Node *node) {
    return insert_internal(
        it, h, k, InsertType::DOES_NOT_EXIST,
        [](const ValueType &) { return false; }, node);
  }
  ...
  bool insert_or_assign(Iterator &it, size_t h, Key &&k, Value &&v) {
    ...
    new (node) Node(cohort_, std::forward<Key>(k), std::forward<Value>(v));
    auto res = insert_internal(
        it, h, node->getItem().first, InsertType::ANY,
        [](const ValueType &) { return false; }, node);
    if (!res) {
      node->~Node();
      Allocator().deallocate((uint8_t *)node, sizeof(Node));
  ...
  bool assign(Iterator &it, size_t h, Key &&k, Value &&v) {
    ...
    new (node) Node(cohort_, std::forward<Key>(k), std::forward<Value>(v));
    auto res = insert_internal(
        it, h, node->getItem().first, InsertType::MUST_EXIST,
        [](const ValueType &) { return false; }, node);
    if (!res) {
      node->~Node();
      Allocator().deallocate((uint8_t *)node, sizeof(Node));
  ...
  bool assign_if(Iterator &it, size_t h, Key &&k, Value &&desired,
                 Predicate &&predicate) {
    auto node = (Node *)Allocator().allocate(sizeof(Node));
    new (node)
        Node(cohort_, std::forward<Key>(k), std::forward<Value>(desired));
    auto res = insert_internal(it, h, node->getItem().first, InsertType::MATCH,
                               std::forward<Predicate>(predicate), node);
    if (!res) {
      node->~Node();
      Allocator().deallocate((uint8_t *)node, sizeof(Node));
    }
  ...
  template <typename Key, typename Value>
  bool assign_if_equal(Iterator &it, size_t h, Key &&k,
                       const ValueType &expected, Value &&desired) {
    return assign_if(it, h, std::forward<Key>(k), std::forward<Value>(desired),
                     [&expected](const ValueType &v) { return v == expected; });
  }

  template <typename MatchFunc, typename K, typename... Args>
  bool insert_internal(Iterator &it, size_t h, const K &k, InsertType type,
                       MatchFunc match, Args &&...args) {
    return impl_.insert(it, h, k, type, match, cohort_,
                        std::forward<Args>(args)...);
  }

  template <typename MatchFunc, typename K, typename... Args>
  bool insert_internal(Iterator &it, size_t h, const K &k, InsertType type,
                       MatchFunc match, Node *cur) {
  ...
  template <typename K> size_type erase(size_t h, const K &key) {
    return erase_internal(h, key, nullptr,
  ...
  template <typename K, typename MatchFunc>
  size_type erase_internal(size_t h, const K &key, Iterator *iter,
                           MatchFunc match) {
