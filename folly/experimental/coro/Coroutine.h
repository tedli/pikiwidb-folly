...
template <typename T = void> class ready_awaitable {
...
template <> class ready_awaitable<void> {
public:
...
//  awaitables.
template <typename... A> class variant_awaitable : private std::variant<A...> {
private:
...
public:
  ...
  virtual std::pair<ExtendedCoroutineHandle, AsyncStackFrame *>
  getErrorHandle(exception_wrapper &) = 0;
...
public:
  ...
  std::pair<ExtendedCoroutineHandle, AsyncStackFrame *>
  getErrorHandle(exception_wrapper &ex) {
    if (extended_) {
...
private:
  ...
  static auto fromBasic(coroutine_handle<Promise> handle) noexcept {
    if constexpr (std::is_convertible_v<Promise *,
                                        ExtendedCoroutinePromise *>) {
...
public:
  ...
  std::pair<ExtendedCoroutineHandle, AsyncStackFrame *>
  getErrorHandle(exception_wrapper &) override {
