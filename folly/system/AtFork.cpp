...
void AtForkList::append(void const *handle, Function<bool()> prepare,
                        Function<void()> parent, Function<void()> child) {
...
void invoke_pthread_atfork(void (*prepare)(), void (*parent)(),
                           void (*child)()) {
  int ret = 0;
...
#endif
  if (ret != 0) {
    throw_exception<std::system_error>(ret, std::generic_category(),
...
void AtFork::init() { AtForkListSingleton::init(); }

void AtFork::registerHandler(void const *handle, Function<bool()> prepare,
                             Function<void()> parent, Function<void()> child) {
