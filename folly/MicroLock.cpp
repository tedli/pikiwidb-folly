...
uint8_t MicroLockCore::lockSlowPath(uint32_t oldWord, detail::Futex<> *wordPtr,
                                    unsigned baseShift, unsigned maxSpins,
...
retry:
  if ((oldWord & heldBit) != 0) {
    ...
    if (spins > maxSpins + maxYields) {
      ...
      if (newWord != oldWord) {
        if (!wordPtr->compare_exchange_weak(oldWord, newWord,
