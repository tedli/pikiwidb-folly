...
template <class Pod, class T> inline void podFill(Pod *b, Pod *e, T c) {
  assert(b && e && b <= e);
 ...
 * adaptation outside).
 */
template <class Pod> inline void podCopy(const Pod *b, const Pod *e, Pod *d) {
  assert(b != nullptr);
  assert(e != nullptr);
  assert(d != nullptr);
  assert(e >= b);
 ...
 * some asserts
 */
template <class Pod> inline void podMove(const Pod *b, const Pod *e, Pod *d) {
  assert(e >= b);
...
private:
  ...
  struct RefCounted {
    ...
    static RefCounted *create(size_t *size) {
      ...
      if (!folly::checked_muladd(&capacityBytes, capacityBytes, sizeof(Char),
                                 getDataOffset())) {
        throw_exception(std::length_error(""));
      }
...
template <class Char> inline Char *fbstring_core<Char>::mutableDataLarge() {
  assert(category() == Category::isLarge);
...
template <class Char>
FOLLY_NOINLINE void
fbstring_core<Char>::reserveMedium(const size_t minCapacity) {
  assert(category() == Category::isMedium);
 ...
 */
template <class Char> class dummy_fbstring_core {
public:
  dummy_fbstring_core() {}
...
template <typename E, class T = std::char_traits<E>,
          class A = std::allocator<E>, class Storage = fbstring_core<E>>
...
public:
  ...
  basic_fbstring(const basic_fbstring &str, size_type pos, size_type n = npos,
                 const A & /* a */ = A()) {
    assign(str, pos, n);
  }
  ...
  typename std::enable_if<
      std::is_convertible<
          TP, typename basic_fbstring<E, T, A, Storage>::value_type>::value &&
          !std::is_same<
...
public:
  ...
  basic_fbstring &replace(size_type pos1, size_type n1,
                          const basic_fbstring &str, size_type pos2,
                          size_type n2) {
    enforce<std::out_of_range>(pos2 <= str.length(), "");
  ...
  basic_fbstring &replace(size_type pos, size_type n1, StrOrLength s_or_n2,
                          NumOrChar n_or_c) {
    Invariant checker(*this);
...
private:
  ...
  basic_fbstring &replaceImplDiscr(iterator i1, iterator i2, size_type n2,
                                   value_type c,
                                   std::integral_constant<int, 1>);

  template <class InputIter>
  basic_fbstring &replaceImplDiscr(iterator i1, iterator i2, InputIter b,
...
public:
  ...
  basic_fbstring &replace(iterator i1, iterator i2, T1 first_or_n_or_s,
                          T2 last_or_c_or_n) {
  ...
  int compare(size_type pos1, size_type n1, const basic_fbstring &str,
...
#if FOLLY_CPLUSPLUS >= 202002L
  friend auto operator<=>(const basic_fbstring &lhs,
                          const basic_fbstring &rhs) {
...
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> &
basic_fbstring<E, T, A, S>::operator=(const basic_fbstring &lhs) {
  Invariant checker(*this);
...
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> &
basic_fbstring<E, T, A, S>::operator=(basic_fbstring &&goner) noexcept {
...
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> &
basic_fbstring<E, T, A, S>::operator=(value_type c) {
  Invariant checker(*this);
...
template <typename E, class T, class A, class S>
inline void
basic_fbstring<E, T, A, S>::resize(const size_type n,
...
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> &
basic_fbstring<E, T, A, S>::append(const basic_fbstring &str) {
#ifndef NDEBUG
...
template <typename E, class T, class A, class S>
FOLLY_NOINLINE basic_fbstring<E, T, A, S> &
...
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> &
basic_fbstring<E, T, A, S>::append(size_type n, value_type c) {
  Invariant checker(*this);
  auto pData = store_.expandNoinit(n, /* expGrowth = */ true);
  fbstring_detail::podFill(pData, pData + n, c);
  return *this;
...
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> &
basic_fbstring<E, T, A, S>::assign(const basic_fbstring &str,
                                   const size_type pos, size_type n) {
  const size_type sz = str.size();
  enforce<std::out_of_range>(pos <= sz, "");
  procrustes(n, sz - pos);
...
basic_fbstring<E, T, A, S>::find(const value_type *needle, const size_type pos,
...
basic_fbstring<E, T, A, S>::insertImplDiscr(const_iterator i, size_type n,
...
template <class InputIter>
inline typename basic_fbstring<E, T, A, S>::iterator
basic_fbstring<E, T, A, S>::insertImplDiscr(const_iterator i, InputIter b,
...
template <class FwdIterator>
inline typename basic_fbstring<E, T, A, S>::iterator
basic_fbstring<E, T, A, S>::insertImpl(const_iterator i, FwdIterator s1,
                                       FwdIterator s2,
                                       std::forward_iterator_tag) {
  Invariant checker(*this);
...
template <class InputIterator>
inline typename basic_fbstring<E, T, A, S>::iterator
basic_fbstring<E, T, A, S>::insertImpl(const_iterator i, InputIterator b,
                                       InputIterator e,
                                       std::input_iterator_tag) {
...
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> &
basic_fbstring<E, T, A, S>::replaceImplDiscr(iterator i1, iterator i2,
                                             const value_type *s, size_type n,
                                             std::integral_constant<int, 2>) {
  assert(i1 <= i2);
...
template <typename E, class T, class A, class S>
template <class FwdIterator>
inline bool basic_fbstring<E, T, A, S>::replaceAliased(iterator i1, iterator i2,
                                                       FwdIterator s1,
                                                       FwdIterator s2,
                                                       std::true_type) {
  std::less_equal<const value_type *> le{};
...
template <typename E, class T, class A, class S>
template <class FwdIterator>
inline void basic_fbstring<E, T, A, S>::replaceImpl(iterator i1, iterator i2,
                                                    FwdIterator s1,
                                                    FwdIterator s2,
                                                    std::forward_iterator_tag) {
  Invariant checker(*this);
...
template <typename E, class T, class A, class S>
inline typename basic_fbstring<E, T, A, S>::size_type
basic_fbstring<E, T, A, S>::rfind(const value_type *s, size_type pos,
                                  size_type n) const {
  if (n > length()) {
    return npos;
  }
...
template <typename E, class T, class A, class S>
inline typename basic_fbstring<E, T, A, S>::size_type
basic_fbstring<E, T, A, S>::find_first_of(const value_type *s, size_type pos,
                                          size_type n) const {
  if (pos > length() || n == 0) {
    return npos;
  }
...
template <typename E, class T, class A, class S>
inline typename basic_fbstring<E, T, A, S>::size_type
basic_fbstring<E, T, A, S>::find_last_of(const value_type *s, size_type pos,
                                         size_type n) const {
  if (!empty() && n > 0) {
...
template <typename E, class T, class A, class S>
inline typename basic_fbstring<E, T, A, S>::size_type
basic_fbstring<E, T, A, S>::find_last_not_of(const value_type *s, size_type pos,
                                             size_type n) const {
  if (!this->empty()) {
...
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S>
operator+(const basic_fbstring<E, T, A, S> &lhs,
...
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S>
operator+(basic_fbstring<E, T, A, S> &&lhs,
          const basic_fbstring<E, T, A, S> &rhs) {
...
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S>
operator+(const E *lhs, const basic_fbstring<E, T, A, S> &rhs) {
  //
  basic_fbstring<E, T, A, S> result;
...
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S>
operator+(E lhs, const basic_fbstring<E, T, A, S> &rhs) {
  basic_fbstring<E, T, A, S> result;
...
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S>
operator+(const basic_fbstring<E, T, A, S> &lhs, const E *rhs) {
...
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S>
operator+(const basic_fbstring<E, T, A, S> &lhs, E rhs) {
  basic_fbstring<E, T, A, S> result;
...
template <typename E, class T, class A, class S>
inline bool
operator==(const typename basic_fbstring<E, T, A, S>::value_type *lhs,
           const basic_fbstring<E, T, A, S> &rhs) {
  return rhs == lhs;
}
...
#define FOLLY_FBSTRING_HASH1(T)                                                \
  template <> struct hash<::folly::basic_fbstring<T>> {                        \
...
template <> struct IsSomeString<fbstring> : std::true_type {};
} // namespace folly
...
struct fmt::formatter<folly::fbstring> : private formatter<fmt::string_view> {
  ...
  typename Context::iterator format(const folly::fbstring &s,
                                    Context &ctx) const {
