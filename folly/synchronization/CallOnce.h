...
try_call_once(OnceFlag &flag, F &&f, Args &&...args) noexcept {
  ...
  return flag.try_call_once_slow(std::forward<F>(f),
...
private:
  ...
  template <typename OnceFlag> friend void reset_once(OnceFlag &) noexcept;

  template <typename OnceFlag> friend void set_once(OnceFlag &) noexcept;
...
static_assert(sizeof(compact_once_flag) == 1,
