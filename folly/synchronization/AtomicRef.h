...
template <typename T> struct atomic_ref_base {
  ...
  bool compare_exchange_strong(T &expected, T desired,
                               ...
                               std::memory_order failure) const noexcept {
    return atomic().compare_exchange_strong(expected, desired, success,
                                            failure);
  }
...
template <typename T> struct atomic_ref_integral_base : atomic_ref_base<T> {
...
using atomic_ref_select =
    conditional_t<std::is_integral<T>::value && !std::is_same<T, bool>::value,
                  atomic_ref_integral_base<T>, atomic_ref_base<T>>;
...
template <typename T> class atomic_ref : public detail::atomic_ref_select<T> {
private:
