...
void *reentrant_allocator_base::allocate(std::size_t const n,
  ...
  while (true) {
    ...
    if (offset + n > block_size || !head) {
      ...
      auto const newhead =
          static_cast<node_t *>(reentrant_allocate(block_size));
...
void reentrant_allocator_base::deallocate(void *const p,
