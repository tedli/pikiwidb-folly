...
public:
  ...
  operator=(reentrant_allocator_base const &that) noexcept;
  ~reentrant_allocator_base();
...
private:
  template <typename> friend class reentrant_allocator;
  ...
  template <typename S> using undeducible = std::enable_if_t<true, S>;
...
public:
  ...
  reentrant_allocator(reentrant_allocator<U> const &that) noexcept
      : base{that} {}
